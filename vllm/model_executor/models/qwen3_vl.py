...
class Qwen3VLProcessingInfo(Qwen2VLProcessingInfo):
    ...
    def _get_max_pixels_for_token_budget(
        self,
        token_budget: int,
        *,
        ...
        unit = vision_config.patch_size * vision_config.spatial_merge_size
        return max(token_budget, 1) * unit * unit * temporal_patch_size

    def _get_effective_mm_token_budget(self, seq_len: int) -> int:
        # Reserve room for fixed wrapper tokens inserted around vision/video
        # placeholders by the Qwen3-VL processor.
        return max(seq_len - self._DUMMY_MM_TEXT_OVERHEAD, 1)
    ...
    def get_mm_max_tokens_per_item(
        ...
        # Respect max_model_len for startup profiling / encoder budgeting.
        # Qwen3-VL family can otherwise derive a maximum-size dummy video that
        # exceeds the configured sequence length and causes startup OOMs.
        effective_budget = self._get_effective_mm_token_budget(seq_len)
        max_image_tokens = min(super().get_max_image_tokens(), effective_budget)
        max_video_tokens = min(
            self.get_max_video_tokens(seq_len, mm_counts), effective_budget
        )
    ...
    def get_max_video_tokens(
        ...
        effective_budget = self._get_effective_mm_token_budget(seq_len)
        video_max_pixels = min(
            video_size["longest_edge"],
            self._get_max_pixels_for_token_budget(
                effective_budget, temporal_patch_size=temporal_patch_size
            ),
        )
...
class Qwen3VLDummyInputsBuilder(BaseDummyInputsBuilder[Qwen3VLProcessingInfo]):
    ...
    def get_dummy_mm_data(
        ...
        image_size = image_mm_kwargs.get("size", image_processor.size)
        effective_budget = self.info._get_effective_mm_token_budget(seq_len)
        image_max_pixels = min(
            image_size["longest_edge"],
            self.info._get_max_pixels_for_token_budget(effective_budget),
        )
        ...
        video_max_pixels = min(
            video_size["longest_edge"],
            self.info._get_max_pixels_for_token_budget(
                effective_budget, temporal_patch_size=temporal_patch_size
            ),
        )
