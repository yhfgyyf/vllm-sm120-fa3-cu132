...
try:
    import flash_attn_interface as _external_fa3_interface

    EXTERNAL_FA3_UNAVAILABLE_REASON = None
    EXTERNAL_FA3_AVAILABLE = True
except ImportError as e:
    EXTERNAL_FA3_UNAVAILABLE_REASON = str(e)
    EXTERNAL_FA3_AVAILABLE = False
...
def _use_external_fa3() -> bool:
    if not EXTERNAL_FA3_AVAILABLE:
        return False
...
def _is_fa3_supported() -> tuple[bool, str | None]:
    ...
    if current_platform.is_device_capability_family(120):
        if EXTERNAL_FA3_AVAILABLE:
            return True, None
        return (
            False,
            "FA3 external backend for SM120 is unavailable due to: "
            f"{EXTERNAL_FA3_UNAVAILABLE_REASON}",
        )

    if not current_platform.is_device_capability_family(90):
        return False, "FA3 is only supported on devices with compute capability 9.x or 12.x"
...
def get_scheduler_metadata(
    ...
    if headdim_v is None:
        headdim_v = headdim
    if _use_external_fa3():
        return _external_fa3_interface.get_scheduler_metadata(
...
def flash_attn_varlen_func(
    ...
    if fa_version == 2:
        ...
        if _use_external_fa3():
            if s_aux is not None:
                raise NotImplementedError(
                    "SM120 external FA3 path does not support s_aux"
                )
            if cp_world_size != 1 or cp_rank != 0 or cp_tot_seqused_k is not None:
                raise NotImplementedError(
                    "SM120 external FA3 path does not support context parallelism"
                )
            out_buffer = out
            out, softmax_lse, _, _ = _external_fa3_interface._flash_attn_forward(
                q,
                k,
                v,
                None,
                None,  # k_new, v_new
                q_v,
                None,  # Let external FA3 allocate outputs to satisfy aliasing checks.
                cu_seqlens_q,
